...
import { getSearchResults, normalizeSearch } from "../lib/search-utils";
import {
  simpleCardClass,
  renderIcon,
  renderResultDetails,
...
function renderValidationRegion(searchValidation) {
...
function renderResultsRegion({ searchQuery, searchResults, hasEnoughChars }) {
...
export function buildSearchInputState(attendees, value) {
...
export function applySearchInput(state, value) {
  const nextState = buildSearchInputState(state.product?.attendees || [], value);
  ...
  renderResultsRegion(nextState);
  renderValidationRegion(state.searchValidation);
  syncClearSearchButton(state.searchQuery);

  return nextState;
}

export function applySearchValidationSuccess(state, validation) {
...
export function applySearchValidationError(state, errorMessage, attemptedAttendee) {
  state.searchValidation = {
    ok: false,
...
export function refreshSearchResults(state) {
